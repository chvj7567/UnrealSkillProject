...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SpyAssetData)

const USpyAssetData& USpyAssetData::Get()
{
	return USpyAssetManager::Get().GetAssetData();
}

#if WITH_EDITOR
void USpyAssetData::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
...
EDataValidationResult USpyAssetData::IsDataValid(FDataValidationContext& Context) const
{
...
FSoftObjectPath USpyAssetData::GetAssetPathByName(const FName& AssetName) const
{
