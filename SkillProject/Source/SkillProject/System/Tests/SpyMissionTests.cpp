...
//# 기획서(docs/design/mission-system.md §3-1) 확정 테이블 픽스처.
...
bool FSpyMissionDesignChainTest::RunTest(const FString& Parameters)
{
	...
	//#    현재 미션이 처치(index 0)이므로 Lv2 신호는 태그 불일치로 버려져야 한다 (§2 경로 B 의 핵심)
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Event_Mission_Level, 2);
	TestEqual(TEXT("Lv2 signal discarded — still on kill mission"), State.MissionIndex, 0);
	...
	TestTrue(TEXT("Mission 0 (kill) completed"), State.bCompletedNow);
	TestEqual(TEXT("Advanced to level mission"), State.MissionIndex, 1);
	...
	TestTrue(TEXT("Mission 1 (level) completed by Lv3 signal"), State.bCompletedNow);
	TestEqual(TEXT("Advanced to combo mission"), State.MissionIndex, 2);
	...
	TestTrue(TEXT("Mission 2 (combo) completed on the 4th"), State.bCompletedNow);
	TestEqual(TEXT("Advanced to vault mission"), State.MissionIndex, 3);
	...
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Skill_Move_Vault, 1);
	TestTrue(TEXT("Mission 3 (vault) completed on the 5th"), State.bCompletedNow);
	TestEqual(TEXT("Advanced to climb mission"), State.MissionIndex, 4);
	...
	TestTrue(TEXT("Mission 4 (climb) completed on the 3rd"), State.bCompletedNow);
	TestEqual(TEXT("Advanced to grapple mission"), State.MissionIndex, 5);
	...
	TestTrue(TEXT("Mission 5 (grapple) completed on the 3rd"), State.bCompletedNow);
	TestEqual(TEXT("Index past the end"), State.MissionIndex, 6);
	TestTrue(TEXT("All missions completed"), State.bAllCompleted);

	//# 완주 후 어떤 이벤트도 상태를 되돌리지 못한다
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Event_Mission_Kill, 1);
	TestEqual(TEXT("Index stays past the end"), State.MissionIndex, 6);
...
bool FSpyMissionDesignTableTest::RunTest(const FString& Parameters)
{
	...
	{
		const FSpyMissionEntry* Entry = Config->GetMission(Index);
		if (Entry == nullptr)
		{
			AddError(FString::Printf(TEXT("Mission %d is missing"), Index));
	...
	const FSpyMissionEntry* LevelEntry = Config->GetMission(1);
...
bool FSpyMissionEventBeforeItsTurnTest::RunTest(const FString& Parameters)
{
	...
	//# 처치 미션 진행 중 레벨 승급 신호가 세 번 도착해도 전부 폐기된다
	State = Config->ResolveMissionProgress(0, 0, SpyGameplayTags::Event_Mission_Level, 2);
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Event_Mission_Level, 3);
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Event_Mission_Level, 4);

	TestEqual(TEXT("Still on the kill mission"), State.MissionIndex, 0);
	...
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Event_Mission_Kill, 1);
	TestEqual(TEXT("Level mission is now current"), State.MissionIndex, 1);
...
bool FSpyMissionThresholdJumpTest::RunTest(const FString& Parameters)
{
	...
	const FSpyMissionProgressResult Result = Config->ResolveMissionProgress(1, 0, SpyGameplayTags::Event_Mission_Level, 4);

	TestTrue(TEXT("Completed by overshooting threshold"), Result.bCompletedNow);
	TestEqual(TEXT("Advanced to combo mission"), Result.MissionIndex, 2);
...
bool FSpyMissionSameTagAfterCompleteTest::RunTest(const FString& Parameters)
{
	...
	FSpyMissionProgressResult State = Config->ResolveMissionProgress(0, 0, SpyGameplayTags::Event_Mission_Kill, 1);
	TestEqual(TEXT("Kill mission completed"), State.MissionIndex, 1);

	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Event_Mission_Kill, 1);
	TestEqual(TEXT("Second kill did not touch the level mission"), State.Count, 0);
	TestEqual(TEXT("Index unchanged"), State.MissionIndex, 1);
	TestFalse(TEXT("Nothing completed"), State.bCompletedNow);

	//# 파쿠르 미션 사이에서도 같은 성질이 성립한다 — 넘기 완료 후 남은 넘기 입력이 벽타기에 새지 않는다
	FSpyMissionProgressResult Parkour = Config->ResolveMissionProgress(3, 4, SpyGameplayTags::Skill_Move_Vault, 1);
	TestEqual(TEXT("Advanced to climb mission"), Parkour.MissionIndex, 4);
...
bool FSpyMissionNonPositiveAmountTest::RunTest(const FString& Parameters)
{
	...
	const FSpyMissionProgressResult Zero = Config->ResolveMissionProgress(2, 1, SpyGameplayTags::Event_Mission_Combo, 0);
	...
	const FSpyMissionProgressResult Negative = Config->ResolveMissionProgress(2, 1, SpyGameplayTags::Event_Mission_Combo, -5);
	...
	const FSpyMissionProgressResult ThresholdZero = Config->ResolveMissionProgress(1, 2, SpyGameplayTags::Event_Mission_Level, 0);
	...
	const FSpyMissionProgressResult ThresholdNegative = Config->ResolveMissionProgress(1, 2, SpyGameplayTags::Event_Mission_Level, -3);
	...
	const FSpyMissionProgressResult NegativeCount = Config->ResolveMissionProgress(2, -4, SpyGameplayTags::Event_Mission_Combo, 1);
...
bool FSpyMissionInvalidMatchTagTest::RunTest(const FString& Parameters)
{
	...
	FSpyMissionProgressResult State;
	State = Config->ResolveMissionProgress(0, 0, SpyGameplayTags::Event_Mission_Kill, 1);
	State = Config->ResolveMissionProgress(State.MissionIndex, State.Count, SpyGameplayTags::Skill_Move_Vault, 1);

	TestEqual(TEXT("Stalled at index 0"), State.MissionIndex, 0);
	...
	const FSpyMissionProgressResult InvalidEvent = Design->ResolveMissionProgress(0, 0, FGameplayTag(), 1);
...
{
	//# 음수 인덱스는 무시가 아니라 0 으로 clamp 된 뒤 정상 판정된다.
	//# (ResolveMissionProgress 의 FMath::Max(0, InIndex)) — 복제 초기화 이전 상태 등에서
	//# 음수가 들어와도 첫 미션으로 되돌아갈 뿐 크래시하지 않는다
	const USpyMissionConfig* Config = SpyMissionTests_MakeDesignConfig();

	const FSpyMissionProgressResult Clamped = Config->ResolveMissionProgress(-1, 0, SpyGameplayTags::Event_Mission_Kill, 1);
	TestTrue(TEXT("Clamped to mission 0 and completed it"), Clamped.bCompletedNow);
	TestEqual(TEXT("Advanced from the clamped index"), Clamped.MissionIndex, 1);

	//# 훨씬 작은 음수도 동일하게 0 으로 clamp 된다
	const FSpyMissionProgressResult DeepNegative = Config->ResolveMissionProgress(-100, 0, SpyGameplayTags::Event_Mission_Combo, 1);
	TestEqual(TEXT("Deep negative clamped to mission 0"), DeepNegative.MissionIndex, 0);
	TestEqual(TEXT("Combo event does not match mission 0"), DeepNegative.Count, 0);
...
bool FSpyMissionIndexBoundaryTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Index 0 valid"), Config->IsValidMissionIndex(0));
	TestTrue(TEXT("Index 5 valid (last)"), Config->IsValidMissionIndex(5));
	TestFalse(TEXT("Index 6 invalid (one past the end)"), Config->IsValidMissionIndex(6));

	const FSpyMissionEntry* First = Config->GetMission(0);
	const FSpyMissionEntry* Last = Config->GetMission(5);

	TestTrue(TEXT("GetMission(-1) is null"), Config->GetMission(-1) == nullptr);
	TestTrue(TEXT("GetMission(0) is valid"), First != nullptr);
	TestTrue(TEXT("GetMission(5) is valid"), Last != nullptr);
	TestTrue(TEXT("GetMission(6) is null"), Config->GetMission(6) == nullptr);
