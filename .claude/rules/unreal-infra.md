...
> 원본 Unity 룰 03(ChvjPackage)·04(에셋) 대체. SpyProject 재사용 인프라(SKGAS·SKAssetCore)와 GAS·DataAsset·모듈 규약을 정의한다.
...
> SKAssetCore(`Plugins/SKAssetCore`, 실존) 는 에셋 매니저 플러그인 — `USKAssetManager`(에셋 매니저 base)·`USKAssetData`(이름→경로 룩업)를 담는다. SpyProject 는 `USpyAssetManager : USKAssetManager` 로 소비.

- 역방향 참조 금지 (SKGAS·SKAssetCore 가 SkillProject 를 참조하지 않음).
- 공통 기능은 SKGAS·SKAssetCore 에 두고 게임 코드에서 중복 구현 금지.
